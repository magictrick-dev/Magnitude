...
operator[](char c)
{
    MAG_ASSERT(isalpha(c)); // Explode if it isn't correct.
...
operator[](i32 idx)
{
    MAG_ASSERT(idx < 52); // Ensure we're in bounds.
...
offset_at(char c)
{
    MAG_ASSERT(isalpha(c)); // Ensures that we properly parsed.
    i32 offset = 0;
...
parse(i32 index, ccptr argument)
{
    ...
    MAG_ASSERT(string_length >= 1);
    MAG_ASSERT(argument[0] == '-');
...
{
    CLI::header();
    std::cout << "Useage: magnitude [view-name]" << std::endl;
    std::cout << "      Magnitude is a graphics rendering engine designed from scratch." << std::endl;
    std::cout << "      By default, magnitude requires a view file to render something." << std::endl;
    std::cout << "      For free-form rendering, use --free-form to trigger this." << std::endl;
    std::cout << std::endl;
    std::cout << "      Magnitude was developed to test and actualize various graphics" << std::endl;
    std::cout << "      rendering APIs, techniques, and ideas with a standardized method" << std::endl;
    std::cout << "      for creating reproducible scenes. Credit for this concept goes to" << std::endl;
    std::cout << "      Dr. Duffin at Northern Illinois University for the rdview file format" << std::endl;
    std::cout << "      and his excellent instruction." << std::endl;
    std::cout << std::endl;
    std::cout << "      Developed and maintained by Christopher DeJong, Github@magictrick-dev." << std::endl;
}
